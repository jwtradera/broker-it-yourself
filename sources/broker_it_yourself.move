...
module overmind::broker_it_yourself {
    ...
    */
    public entry fun init(admin: &signer) {
        ...
        let (resource_acc, signer_cap) = account::create_resource_account(admin, SEED);
        ...
        move_to(
            ...
            State {
                ...
                create_offer_events: account::new_event_handle<CreateOfferEvent>(&resource_acc),
                accept_offer_events: account::new_event_handle<AcceptOfferEvent>(&resource_acc),
                complete_transaction_events: account::new_event_handle<CompleteTransactionEvent>(&resource_acc),
                release_funds_events: account::new_event_handle<ReleaseFundsEvent>(&resource_acc),
                cancel_offer_events: account::new_event_handle<CancelOfferEvent>(&resource_acc),
                open_dispute_events: account::new_event_handle<OpenDisputeEvent>(&resource_acc),
                resolve_dispute_events: account::new_event_handle<ResolveDisputeEvent>(&resource_acc),
            }
        );
    }
    ...
    */
    public entry fun create_offer(
        ...
        let offer_exists = simple_map::contains_key(&state.creators_offers, &creator_addr);
        if (offer_exists) {
            let offer_ids = simple_map::borrow_mut(&mut state.creators_offers, &creator_addr);
            vector::push_back(offer_ids, offer_id);
        }
        else {
            let offer_ids: vector<u128> = vector::empty();
            vector::push_back(&mut offer_ids, offer_id);
            simple_map::add(&mut state.creators_offers, creator_addr, offer_ids);
        };
        ...
        if (sell_apt) {
            ...
            coin::transfer<AptosCoin>(creator, resource_acc_addr, apt_amount);
        };

        // TODO: Emit CreateOfferEvent event
        let new_offer_event = broker_it_yourself_events::new_create_offer_event(
            offer_id, 
            creator_addr,
            arbiter,
            apt_amount,
        ...
        event::emit_event<CreateOfferEvent>(
            &mut state.create_offer_events, new_offer_event
        );
    }
    ...
    */
    public entry fun accept_offer(user: &signer, offer_id: u128) acquires State {
        ...
        event::emit_event<AcceptOfferEvent>(
            &mut state.accept_offer_events, accept_offer_event
        );
    }
    ...
    */
    public entry fun complete_transaction(user: &signer, offer_id: u128) acquires State {
        ...
        event::emit_event<CompleteTransactionEvent>(
            &mut state.complete_transaction_events, complete_event
        );
        ...
        if(offer.completion.creator && offer.completion.counterparty) {
            ...
            event::emit_event<ReleaseFundsEvent>(
                &mut state.release_funds_events, release_funds_event
            );
        };
    }
    ...
    */
    public entry fun cancel_offer(creator: &signer, offer_id: u128) acquires State {
        ...
        event::emit_event<CancelOfferEvent>(
            &mut state.cancel_offer_events, cancel_offer_event
        );
    }
    ...
    */
    public entry fun open_dispute(user: &signer, offer_id: u128) acquires State {
        ...
        let open_dispute_event = broker_it_yourself_events::new_open_dispute_event(
            offer_id, 
            user_addr,            
            timestamp::now_seconds()
        );
        event::emit_event<OpenDisputeEvent>(
            &mut state.open_dispute_events, open_dispute_event
        );
    }
    ...
    */
    public entry fun resolve_dispute(
        ...
        if(transfer_to_creator) {
            coin::transfer<AptosCoin>(&resource_account, creator, offer.apt_amount);   
        }
        else {
            if(option::is_some(&offer.counterparty)) {
                let counterparty = option::borrow(&offer.counterparty);
                coin::transfer<AptosCoin>(&resource_account, *counterparty, offer.apt_amount);   
            }
        };
        ...
        let resolve_dispute_event = broker_it_yourself_events::new_resolve_dispute_event(
            offer_id, 
            transfer_to_creator,            
            timestamp::now_seconds()
        );
        event::emit_event<ResolveDisputeEvent>(
            &mut state.resolve_dispute_events, resolve_dispute_event
        );
    }
    ...
    */
    ...
    public fun get_available_offers(): SimpleMap<u128, Offer> acquires State {
        ...
        let offer_ids = simple_map::keys(&state.offers);
        let available_offers = simple_map::create();

        vector::for_each_ref(&offer_ids, |offer_id| {
            let offer = simple_map::borrow(&state.offers, offer_id);
    ...
    */
    ...
    public fun get_buy_offers(creator: address): SimpleMap<u128, Offer> acquires State {
        ...
        let offer_ids = simple_map::borrow(&state.creators_offers, &creator);
    ...
    */
    ...
    public fun get_creator_offers(creator: address): SimpleMap<u128, Offer> acquires State {
        ...
        vector::for_each_ref(&offer_ids, |offer_id| {
            let offer = simple_map::borrow(&state.offers, offer_id);
            let is_creator = offer.creator == creator;
            
            if(!is_creator) {
                let is_have_counterparty = option::is_some(&offer.counterparty);
                if(is_have_counterparty) {
                    let counterparty = option::borrow(&offer.counterparty);
    ...
    */
    public(friend) inline fun remove_offer_from_creator_offers(
        ...
        let offer_ids = simple_map::borrow_mut(creators_offers, creator);
        let (is_exists, offer_idx) = vector::index_of(offer_ids, offer_id);
        if(is_exists) {
    ...
    */
    ...
    inline fun assert_user_has_enough_funds<CoinType>(user: address, coin_amount: u64) {
        ...
        let balance = coin::balance<CoinType>(user);
    ...
    inline fun assert_user_participates_in_transaction(user: address, offer: &Offer) {
        ...
        let is_creator = user == offer.creator;
        let is_counterparty = user == *option::borrow(&offer.counterparty);
        assert!(is_creator || is_counterparty, ERROR_USER_DOES_NOT_PARTICIPATE_IN_TRANSACTION);
    }
    ...
    inline fun assert_user_has_not_marked_completed_yet(user: address, offer: &Offer) {
        ...
        let counterparty = *option::borrow(&offer.counterparty);
        assert!(!offer.completion.creator || !(offer.completion.creator && offer.creator == user), ERROR_USER_ALREADY_MARKED_AS_COMPLETED);
        assert!(!offer.completion.counterparty || !(offer.completion.counterparty && counterparty == user), ERROR_USER_ALREADY_MARKED_AS_COMPLETED);
    }
